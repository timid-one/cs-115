...
def play_nim():
    ...
    while True:
        ...
        display_piles()
        if sum(piles) == 0:
            print("As expected, I have won. " +
                  "No mortal can surpass the great NimMaster5000!")
            break
...
def init_piles():
    ...
    num_piles = int(input("How many piles shall we play with? "))
    while num_piles <= 1:
        if num_piles == 1:
            print("You are not allowed to take advantage of my generosity.")
            num_piles=int(input("Choose a different number. "))
        else:
...
def get_pile():
    ...
    while piles[chosen] == 0:
        chosen = int(input("That pile is empty! Try again. "))
    return chosen
