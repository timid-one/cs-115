...
def wordScoreLoop(S, scorelist):
    ...
    score = 0
    for letter in S:
        score = score + letterScoreLoop(letter, scorelist)
    return score
...
def wordsWithScoreLambda(dct, scores):
    '''Same as wordsWithScore, but with a lambda'''
...
def wordsWithScoreLoop(dct, scores):
    ...
    wordsWithScores = []
    for word in dct:
        wordScore = wordScoreLoop(word, scores)
        wordsWithScores = wordsWithScores + [[word, wordScore]]
